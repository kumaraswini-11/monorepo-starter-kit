...
const noRawAuthClient = {
  ...
  message:
    "Import the auth client only in lib/auth-client.ts; the rest of the app goes through the seam at lib/auth/actions.ts (ADR 0017 §1).",
};
const noSeamBypass = {
  group: ["@/lib/auth-client"],
  message:
    "Use the seam wrappers in lib/auth/actions.ts (enumeration-safe error mapping), not the raw auth client (ADR 0017 §1).",
};

 ...
 * compliance-bound repo — a stray `DATABASE_URL` in the shell must not be silently wiped. (ADR 0029)
 */
  ...
  if (!LOOPBACK_HOSTS.has(host)) {
    throw new Error(
      `Refusing a destructive DB reset against non-local host "${host}". Point DATABASE_URL ` +
        "at a disposable/local database, or set ALLOW_DESTRUCTIVE_DB=1 if you are certain. (ADR 0029)"
    );
  }
}
 ...
 * `assertDisposableDatabase`; never point it at a database whose data you care about. (ADR 0029)
 */

  ...
  return (
    <AlertDialogPortal>
      ...
      {/* Enter/exit: Base UI CSS transition — fade + scale from center (data-starting/ending-style),
          not tw-animate-css keyframes. ADR 0030. */}
      <AlertDialogPrimitive.Popup
        data-slot="alert-dialog-content"
        data-size={size}
        className={cn(
          "group/alert-dialog-content fixed start-1/2 top-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-6 rounded-xl bg-popover p-6 text-popover-foreground ring-1 ring-foreground/10 transition-[opacity,scale] duration-150 ease-snappy outline-none data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0 data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-lg rtl:translate-x-1/2",
          className
        )}
        {...props}
      />

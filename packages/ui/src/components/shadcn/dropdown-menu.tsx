    ...
    <MenuPrimitive.Portal>
      ...
      >
        {/* Enter/exit: Base UI CSS transitions (data-starting/ending-style), not tw-animate-css
            keyframes. Re-apply if `shadcn add --overwrite` restores keyframes. ADR 0030. */}
        <MenuPrimitive.Popup
          data-slot="dropdown-menu-content"
          className={cn(
            "z-50 max-h-(--available-height) w-(--anchor-width) min-w-32 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-md bg-popover p-1 text-popover-foreground shadow-md ring-1 ring-foreground/10 transition-[opacity,scale] duration-150 ease-snappy outline-none data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0 data-closed:overflow-hidden",
            className
          )}
          {...props}
        />
...
function DropdownMenuSubContent({
  ...
  // Inherits the enter/exit transition from DropdownMenuContent (ADR 0030); only layout here.
  return (
    <DropdownMenuContent
